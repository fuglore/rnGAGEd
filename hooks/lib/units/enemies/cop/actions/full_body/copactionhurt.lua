...
Hooks:PostHook(CopActionHurt, "init", "reeg_funny_enter", function(self, action_desc, common_data)
	if Network:is_server() then
		if self._shooting_hurt or self._delayed_shooting_hurt_clbk_id then
			self._ext_movement:set_friendly_fire(true)
			self._friendly_firing = true
		end
	end
end)

Hooks:PostHook(CopActionHurt, "exit", "reeg_funny_exit", function(self)
	if Network:is_server() then
		if self._friendly_firing then
			self._ext_movement:set_friendly_fire(false)
			self._friendly_firing = false
		end
	end
end)
